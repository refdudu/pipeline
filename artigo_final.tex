...
A análise primária aplica o Teste de Friedman ao efeito principal da persona, separadamente para cada métrica de qualidade, com análise \textit{post-hoc} de Nemenyi para as comparações par a par. A escolha de testes não-paramétricos justifica-se pela natureza pareada dos dados e pela não-normalidade esperada das saídas do modelo. A análise da interação entre persona e tipo de refatoração emprega ANOVA sobre postos alinhados (\textit{Aligned Rank Transform}) \cite{wobbrock2011}, método não-paramétrico que admite desenhos fatoriais; seus resultados são exploratórios, dado o poder estatístico limitado da amostra estratificada. Adota-se nível de significância de $\alpha = 0{,}05$.
...
A Tabela II reporta a taxa de preservação de comportamento e o custo médio em tokens por persona. As taxas de validade são altas e próximas entre si, e o Teste Q de Cochran não detecta diferença de atrito entre as personas ($Q = 2{,}40$; $p = 0{,}494$). Ou seja, a persona de controle negativo (o padeiro, fora de domínio) não quebrou mais código do que a persona especializada. Quanto ao custo, a persona especializada consumiu, em média, praticamente os mesmos tokens que o prompt neutro (7665 contra 7646), ao passo que a persona de controle negativo foi a mais custosa (8267), por tender a acrescentar texto fora da tarefa.
...
\hline
P-1 (controle neg.) & 59/60 & 0,983 & 8267 \\
\hline
P0 (neutro) & 60/60 & 1,000 & 7646 \\
\hline
P1 (genérico) & 59/60 & 0,983 & 7605 \\
\hline
P2 (especializado) & 58/60 & 0,967 & 7665 \\
\hline
\end{tabularx}
\end{table}

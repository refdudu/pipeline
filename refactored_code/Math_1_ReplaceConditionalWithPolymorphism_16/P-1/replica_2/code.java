...
public class TriangularDistribution extends AbstractRealDistribution {
    ...
    /** Segment polymorphs for density and cumulative probability. */
    private final Segment[] segments;
    /** Segment polymorphs for inverse cumulative probability. */
    ...
    public TriangularDistribution(RandomGenerator rng,
        ...
        this.segments = new Segment[] {
            new BelowLowerBound(),
            new BetweenLowerAndMode(),
            new AtMode(),
            new BetweenModeAndUpper(),
            new AboveUpperBound()
        };
        ...
        this.pSegments = new ProbabilitySegment[] {
            new ProbabilityZero(),
            new ProbabilityOne(),
            new ProbabilityBelowModeThreshold(),
            new ProbabilityAboveModeThreshold()
        };
    }
    ...
    public double cumulativeProbability(double x)  {
        for (Segment segment : segments) {
            if (segment.contains(x)) {
                return segment.getCumulativeProbability(x);
            }
        }
        return 1;
    }
    ...
    private abstract class Segment {
        ...
        abstract double getCumulativeProbability(double x);
    }

    private class BelowLowerBound extends Segment {
        @Override
    ...
    private class BetweenLowerAndMode extends Segment {
        @Override
        boolean contains(double x) {
            return a <= x && x < c;
        }
    ...
    private class AtMode extends Segment {
        @Override
        boolean contains(double x) {
            return x == c;
        }
    ...
    private class BetweenModeAndUpper extends Segment {
        @Override
        boolean contains(double x) {
            return c < x && x <= b;
        }
    ...
    private class AboveUpperBound extends Segment {
        @Override
        boolean contains(double x) {
            return true;
        }
    ...
    private abstract class ProbabilitySegment {
        ...
        abstract double getValue(double p);
    }

    private class ProbabilityZero extends ProbabilitySegment {
        @Override
    ...
    private class ProbabilityOne extends ProbabilitySegment {
        @Override
        boolean contains(double p) {
            return p == 1;
        }
    ...
    private class ProbabilityBelowModeThreshold extends ProbabilitySegment {
        @Override
        boolean contains(double p) {
            return p < (c - a) / (b - a);
        }
    ...
    private class ProbabilityAboveModeThreshold extends ProbabilitySegment {
        @Override
        boolean contains(double p) {
            return true;
        }

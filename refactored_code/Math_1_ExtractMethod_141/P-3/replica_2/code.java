...
public class RiddersSolver extends AbstractUnivariateSolver {
    ...
    protected double doSolve()
        ...
        while (true) {
            ...
            final double correction = computeCorrection(interval.x1, x3, interval.y1, interval.y2, y3);
            final double x = x3 - correction;
            final double y = computeObjectiveValue(x);

            // check for convergence
            if (isConverged(x, oldx, y, absoluteAccuracy, relativeAccuracy, functionValueAccuracy)) {
                return x;
            }
    ...
    /**
     * Checks if the convergence criteria are met.
     */
    private boolean isConverged(double x, double oldx, double y,
                                double absoluteAccuracy, double relativeAccuracy, double functionValueAccuracy) {
        final double tolerance = FastMath.max(relativeAccuracy * FastMath.abs(x), absoluteAccuracy);
        return FastMath.abs(x - oldx) <= tolerance || FastMath.abs(y) <= functionValueAccuracy;
    }

    /**
     * Updates the bracketing interval using Ridders' method logic.
     */
    ...
    /**
     * Inner class to represent the bracketing interval.
     */

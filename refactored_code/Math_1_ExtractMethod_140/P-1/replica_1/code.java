...
public class MullerSolver2 extends AbstractUnivariateSolver {
    ...
    protected double doSolve()
        ...
        if (isRoot(y0, functionValueAccuracy)) {
            return x0;
        }
        double x1 = max;
        double y1 = computeObjectiveValue(x1);
        if (isRoot(y1, functionValueAccuracy)) {
            return x1;
        }
        ...
        while (true) {
            ...
            if (denominator != 0) {
                ...
                x = perturb(x, x1, x2, absoluteAccuracy);
            } else {
    ...
    /**
     * Check if the function value is close enough to zero to be considered a root.
     */
    private boolean isRoot(double y, double functionValueAccuracy) {
        return FastMath.abs(y) < functionValueAccuracy;
    }

    /**
     * Verify that the function values at the endpoints have different signs.
     */
    ...
    /**
     * Compute the denominator for Muller's method update formula.
     */
    ...
    /**
     * Perturb x if it exactly coincides with x1 or x2 to avoid division by zero or stagnation.
     */
    private double perturb(double x, double x1, double x2, double absoluteAccuracy) {
        while (x == x1 || x == x2) {
            x += absoluteAccuracy;
        }
        return x;
    ...
    /**
     * Check if the approximation has converged to a root.
     */
